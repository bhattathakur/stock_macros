...
# [WSJ UPGRADES & DOWNGRADES](https://www.wsj.com/market-data/stocks/upgradesdowngrades)
...
choose=['TICKER', 'CP', 'RSI','VOL', 'AVGVOL', '%CHG', 'ATR',
       'SECTOR', 'INDUSTRY', 'CATEGORY']
df=pd.read_csv('/home/thakur/test/combined/all_nasdaq.csv')[choose]
category=['mega','large','medium','small']
df=df[(df.CATEGORY.isin(category)) & (df.CP>5) & (df.VOL>0.1) & (df['%CHG']!=np.inf)]
df.insert(loc=7,column='ATR/CP',value=round(df['ATR']/df['CP']*100,2))
df.head(20)
...
df_atr_up=df.sort_values(by=['ATR'],ascending=False).loc[:,['TICKER','ATR','CP']].head(rows)
df_atr_down=df.sort_values(by=['ATR'],ascending=True).loc[:,['TICKER','ATR','CP']].head(rows)
...
#atr
bars=axs[1,1].bar(df_atr_up['TICKER'],df_atr_up['ATR'],color='g')
